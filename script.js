...
function toggleCaller() {
  const panel = document.getElementById("caller-panel");
  panel.classList.toggle("open");
}

function setCallerActive(el) {
  document
    .querySelectorAll(".caller-option")
    .forEach((opt) => opt.classList.remove("active"));
  el.classList.add("active");
  // Don't prevent default — allow the tel: link to fire
}

// Close caller panel when clicking outside
document.addEventListener("click", (e) => {
  const widget = document.getElementById("caller-widget");
...
// --- ONE VIDEO AT A TIME ---
// Pause all other videos whenever one starts playing
document.addEventListener("DOMContentLoaded", () => {
...
// Poll for new messages every 5 seconds (started after DOM ready)
...
document.addEventListener("DOMContentLoaded", async function () {
  // Initialize Supabase first so map markers can load
  initSupabase();
...
// Load jobs and start message polling after DOM is ready
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentSlide = 0;
  const slideInterval = 5000; // Advances every 5 seconds
  ...
  // Start the interval if slides exist
  if (slides.length > 0) {
